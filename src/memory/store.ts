...
export class MemoryStore {
  ...
  /** Store a new fact */
  addFact(content: string, tags: string[] = [], sourceSession: string = ''): StoredFact {
  ...
  getSession(id: string): StoredSession | null {
    const query = this.db.query(`SELECT id, summary, created_at as createdAt FROM sessions WHERE id = ?`);
    const result = query.get(id) as any;
    return result || null;
  }
  
  /** Get recent sessions */
  getRecentSessions(limit: number = 10): StoredSession[] {
    const query = this.db.query(`SELECT id, summary, created_at as createdAt FROM sessions ORDER BY created_at DESC LIMIT ?`);
