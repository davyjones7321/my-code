  ...
  async summarizeSession(
    ...
    if (provider) {
      ...
      const toolCalls = messages
        ...
        .flatMap(m => m.content.filter(c => c.type === 'tool_use').map(c => c.type === 'tool_use' ? c.name : ''));
      const uniqueTools = [...new Set(toolCalls)];
      
      const firstUser = messages.find(m => m.role === 'user')?.content.find(c => c.type === 'text');
      const firstUserText = firstUser && firstUser.type === 'text' ? firstUser.text : '';
      const lastUser = [...messages].reverse().find(m => m.role === 'user')?.content.find(c => c.type === 'text');
