...
describe('ApprovalGate', () => {
  it('yolo mode approves everything', () => {
    ...
    expect(gate.check({ toolName: 'run_command', toolInput: { command: 'rm -rf /' } })).toBe('approve');
  });
  ...
  it('auto mode blocks rm -rf / with reason', () => {
    ...
    expect(gate.check({ toolName: 'run_command', toolInput: { command: 'rm -rf /' } })).toBe('deny');
  });

  it('auto mode blocks git push --force', () => {
    const gate = new ApprovalGate('auto');
    expect(gate.check({ toolName: 'run_command', toolInput: { command: 'git push origin main --force' } })).toBe('deny');
  });

  it('auto mode approves normal commands (echo hello, npm test)', () => {
    const gate = new ApprovalGate('auto');
    expect(gate.check({ toolName: 'run_command', toolInput: { command: 'echo hello' } })).toBe('approve');
    expect(gate.check({ toolName: 'run_command', toolInput: { command: 'npm test' } })).toBe('approve');
  });
