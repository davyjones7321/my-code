...
describe("ContextCompactor", () => {
	...
	it("estimateTokens returns reasonable estimates", () => {
		...
		const msgs: Message[] = [{ role: "user", content: [{ type: "text", text: "1234" }] }];
		// 4 chars = ~1 token
	...
	it("mechanical summary extracts tool names and text snippets", async () => {
		...
		const msgs: Message[] = [
			...
			{
				role: "assistant",
				content: [{ type: "text", text: "this is a very long text".repeat(10) }],
			},
			{
				role: "assistant",
				content: [{ type: "tool_use", id: "1", name: "myTool", input: { a: 1 } }],
			},
