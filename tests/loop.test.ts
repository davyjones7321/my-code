...
import { runAgentLoop, type ToolExecutor } from '../src/agent/loop.ts';
...
describe('runAgentLoop', () => {
  ...
  it('Tool use cycle: tool_use -> tool executes -> response', async () => {
    ...
    expect(events[0]).toEqual({ type: 'tool_call', toolName: 'echo', toolInput: { message: 'hi' }, toolUseId: '1' });
    expect(events[1]).toEqual({ type: 'tool_result', toolUseId: '1', result: 'echoed: hi', isError: false });
  ...
  it('Tool error handling', async () => {
    ...
    expect(events[1]).toEqual({ type: 'tool_result', toolUseId: '1', result: 'an error occurred', isError: true });
