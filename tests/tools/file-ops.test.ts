...
describe('File Operations Tools', () => {
  ...
  describe('edit_file', () => {
    it('replaces text correctly, returns diff', async () => {
      ...
      const res = await tool.execute({ path: 'test.txt', oldText: 'line 3', newText: 'line THREE' });
      expect(res.isError).toBe(false);
    ...
    it('returns error when old text not found', async () => {
      const tool = createFileEditTool(tmpDir);
      const res = await tool.execute({ path: 'test.txt', oldText: 'missing text', newText: 'new text' });
      expect(res.isError).toBe(true);
